...
## Overview
Infomaniak Rich HTML Editor — an Android library for displaying and editing HTML inside a `WebView` using the `contenteditable` attribute. Published on **Maven Central** and **Reposilite** (`maven.infomaniak.app`) as `com.infomaniak.richhtmleditor:android-rich-html-editor`. No Hilt, no Compose. Kotlin + XML.
